<?php
/**
 * EDD Ajax Search Pro Activation Handler
 *
 * @package     EDD\Ajax_Search_Pro\Classes\Activation
...
if ( ! class_exists( 'EDD_Ajax_Search_Activation' ) ) {
     ...
     * @since       1.0.0
     */
    class EDD_Ajax_Search_Activation {
        ...
        public function missing_edd_notice() {
            if( $this->has_edd ) {
                ...
                $link = '<a href="' . $url . '">' . __( 'activate it', 'edd-ajax-search' ) . '</a>';
            } else {
                ...
                $link = '<a href="' . $url . '">' . __( 'install it', 'edd-ajax-search' ) . '</a>';
            }

            echo '<div class="error"><p>' . $this->plugin_name . sprintf( __( ' requires Easy Digital Downloads! Please %s to continue!', 'edd-ajax-search' ), $link ) . '</p></div>';
        }
